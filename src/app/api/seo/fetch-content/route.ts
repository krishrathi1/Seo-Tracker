  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return NextResponse.json({ error: `Failed to fetch page content: ${response.statusText}` }, { status: 400 })
      }
    } catch (fetchErr) {
      console.error('Fetch content fetch error:', fetchErr)
      return NextResponse.json({ error: 'Failed to read page content directly' }, { status: 400 })
    }
